  ...
  const startLocationWatch = async () => {
    ...
    locationWatchRef.current = await Location.watchPositionAsync(
      ...
      (position) => {
        ...
        centerMap(latitude, longitude, 16);
  ...
  useEffect(() => {
    ...
    const initLocation = async () => {
      try {
        ...
        try {
          await getCurrentLocation(Location.Accuracy.Balanced, 3000);
        } catch (error) {
          console.warn('Ошибка быстрого определения геопозиции:', error);
        }
        try {
          await getCurrentLocation(Location.Accuracy.High, 8000);
        } catch (error) {
          console.warn('Ошибка точного определения геопозиции:', error);
        }
