...
const NextBlockScreen: React.FC<NextBlockScreenProps> = ({
  ...
  const currentBlockIndex = DEFAULT_BLOCKS.findIndex(block => block.id === blockId);
  ...
  const nextBlock = nextBlockIndex < DEFAULT_BLOCKS.length 
    ? DEFAULT_BLOCKS[nextBlockIndex] 
    : null;
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.startButtonContainer}>
        ...
        >
          <Text style={styles.startButtonText}>Продолжить диагностику</Text>
...
const BlockGaugeSimple: React.FC<BlockGaugeSimpleProps> = ({
  ...
  const greenAngle = isGray ? 0 : (totalAnswers > 0 ? (correctAnswers / totalAnswers) * TOTAL_ANGLE : 0);
  const redAngle = isGray ? 0 : (totalAnswers > 0 ? (incorrectAnswers / totalAnswers) * TOTAL_ANGLE : TOTAL_ANGLE);
