...
export default function DiagnosisQuestionsScreen({ 
  ...
  // Если блоки не переданы, используем все блоки по умолчанию в порядке DEFAULT_BLOCKS
  ...
  useEffect(() => {
    const loadSavedAnswers = async () => {
      try {
        const saved = await AsyncStorage.getItem(`diagnosis_answers_${currentBlockId}`);
        if (saved) {
  ...
  const saveAnswer = async (questionId: string, answerId: string) => {
    try {
      ...
      await AsyncStorage.setItem(`diagnosis_answers_${currentBlockId}`, JSON.stringify(updatedAnswers));
