...
const BlockResultsScreen: React.FC<BlockResultsScreenProps> = ({
  ...
  const [blockEfficiency, setBlockEfficiency] = useState<number | null>(null);
  ...
  const resolvedIndexFromSelection = blocksToUse.indexOf(blockId);
  const fallbackIndex = DEFAULT_BLOCKS.findIndex(block => block.id === blockId);
  const currentBlockIndex = completedBlockIndex !== undefined
    ? completedBlockIndex
    : (resolvedIndexFromSelection >= 0 ? resolvedIndexFromSelection : fallbackIndex);
    ...
    const loadAnswersAndCalculateEfficiency = async () => {
      try {
        ...
        if (saved) {
          ...
          const totalQuestions = blockQuestions.length || 0;
          const efficiency = totalQuestions > 0 ? Math.round((scoreSum / totalQuestions) * 100) : 0;
          ...
          if (venueId) {
            const perVenueSaved = await AsyncStorage.getItem(perVenueKey);
            if (perVenueSaved) {
              const perVenueParsed = JSON.parse(perVenueSaved);
              storedResult = perVenueParsed?.[venueId]?.[blockId] || null;
            }
          }
          if (storedResult) {
            setBlockEfficiency(typeof storedResult.efficiency === 'number' ? storedResult.efficiency : 0);
            setCorrectAnswers(typeof storedResult.correct === 'number' ? storedResult.correct : 0);
            setIncorrectAnswers(typeof storedResult.incorrect === 'number' ? storedResult.incorrect : 0);
            setTasksCount(typeof storedResult.tasksCount === 'number' ? storedResult.tasksCount : 0);
          }
        }
             } catch (error) {
    ...
    const saveBlockResult = async () => {
      ...
      try {
        ...
        const perVenueKey = userId
          ? `user_${userId}_diagnosis_block_results_by_venue`
          : 'diagnosis_block_results_by_venue';
        const venueKey = userId
          ? `user_${userId}_diagnosis_selected_venue_id`
          : 'diagnosis_selected_venue_id';
        const venueId = await AsyncStorage.getItem(venueKey);
        const resultPayload = {
          blockId,
        ...
        if (venueId) {
          const perVenueSaved = await AsyncStorage.getItem(perVenueKey);
          const perVenueParsed = perVenueSaved ? JSON.parse(perVenueSaved) : {};
          const venueResults = perVenueParsed?.[venueId] || {};
          perVenueParsed[venueId] = { ...venueResults, [blockId]: resultPayload };
          await AsyncStorage.setItem(perVenueKey, JSON.stringify(perVenueParsed));
        } else {
          const key = userId ? `user_${userId}_diagnosis_block_results` : 'diagnosis_block_results';
          const saved = await AsyncStorage.getItem(key);
          const parsed = saved ? JSON.parse(saved) : {};
          parsed[blockId] = resultPayload;
          await AsyncStorage.setItem(key, JSON.stringify(parsed));
        }
      } catch (error) {
        console.error('Ошибка сохранения результатов блока:', error);
      }
    };

    if (blockEfficiency !== null) {
      saveBlockResult();
    }
  }, [blockId, blockEfficiency, correctAnswers, incorrectAnswers, tasksCount]);
      ...
      <View style={styles.contentContainer}>
        ...
        <View style={[styles.section, styles.efficiencySection]}>
          ...
          <AnimatedPressable 
            ...
            onPress={onViewTasks || onContinue}
          >
...
const BlockGaugeSimple: React.FC<BlockGaugeSimpleProps> = ({
  ...
  // Вычисляем углы секций по эффективности (жёлтые ответы учитываются)
  const clampedEfficiency = Math.max(0, Math.min(100, efficiency));
  const greenAngle = (clampedEfficiency / 100) * TOTAL_ANGLE;
  const redAngle = TOTAL_ANGLE - greenAngle;
