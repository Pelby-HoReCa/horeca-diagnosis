...
export default function RegisterScreen2({
  ...
  const [restaurantLayouts, setRestaurantLayouts] = useState<Record<number, number>>({});
  const [lastMapTargetIndex, setLastMapTargetIndex] = useState<number | null>(null);
  const [shouldScrollToMapTarget, setShouldScrollToMapTarget] = useState(false);
  ...
  useEffect(() => {
    ...
    const loadSavedRegistration = async () => {
      ...
      console.log('Регистрация шаг 2: сохраненные данные', saved);
      if (saved) {
        try {
  ...
  useEffect(() => {
    if (mapTargetIndex === null || mapTargetIndex === undefined) {
      return;
    }
    setLastMapTargetIndex(mapTargetIndex);
    setShouldScrollToMapTarget(true);
  }, [mapTargetIndex]);

  useEffect(() => {
    if (lastMapTargetIndex === null || lastMapTargetIndex === undefined) {
      return;
    }
    if (!hasLoadedRegistration) {
      return;
    }
    if (!shouldScrollToMapTarget) {
      return;
    }
    const targetY = restaurantLayouts[lastMapTargetIndex];
    if (targetY === undefined) {
      return;
    }
    const offsetY = Math.max(targetY - 16, 0);
    setTimeout(() => {
      scrollViewRef.current?.scrollTo({ y: offsetY, animated: true });
    }, 50);
    setShouldScrollToMapTarget(false);
  }, [lastMapTargetIndex, restaurantLayouts, hasLoadedRegistration, shouldScrollToMapTarget]);
  ...
  const handleContinue = async () => {
    ...
    restaurants.forEach((restaurant) => {
      if (!restaurant.address.trim()) {
        nextAddressErrors[restaurant.id] = 'Укажите город и адрес';
        return;
      }
      if (!hasCityInAddress(restaurant.address)) {
        nextAddressErrors[restaurant.id] = 'Добавьте город';
      }
    });
  ...
  return (
    <View style={styles.container}>
      <TouchableWithoutFeedback
        accessible={false}
        onPress={() => {
          if (insideInputTouchRef.current || isSuggestionInteracting || isScrollInProgress) {
            return;
          }
            ...
            {restaurants.map((restaurant, index) => (
              <View
                ...
                onLayout={(event) => {
                  ...
                  setRestaurantLayouts((prev) => ({ ...prev, [index]: y }));
                }}
              >
                {/* Названия заведения */}
                <View
                ...
                >
                  ...
                  {!!addressErrors[restaurant.id] && (
                    <Text style={styles.errorText}>{addressErrors[restaurant.id]}</Text>
                  )}
