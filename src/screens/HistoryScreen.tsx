...
import { getCurrentUserId, getSelectedVenueId, getVenueScopedKey, loadUserBlocks, loadUserTasks } from '../utils/userDataStorage';
...
export default function HistoryScreen() {
  ...
  const fetchBlocksAndTasks = async (): Promise<{ completedBlocks: DiagnosisResult[]; tasksList: Task[] }> => {
    ...
    try {
      ...
      if (blocksSource.length === 0 && !userId) {
      ...
      if (tasksSource.length === 0 && !userId) {
        const venueId = await getSelectedVenueId(userId);
